    ...
    if defrect == False:
        ...
        for i in range(len(Point)):
            ...
            path = path + [astar(labi, (Point[i].rect.x // taillecarre, Point[i].rect.y // taillecarre),
                                 ...
                                 pointtot)]
            pointfait += 1
        # établi que la première personne est infectés
        ...
        for i in actif:  # mouvement des points
            if Point[i].centre != [path[i][Point[i].compteur + 1][0] * taillecarre + taillecarre / 2,path[i][Point[i].compteur + 1][1] * taillecarre + taillecarre / 2] and Point[i].pause[0]==False:
